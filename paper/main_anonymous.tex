...
We propose the EcoTexture AI framework \citep{alaa2026ecotexture}. The core scientific hypothesis is that incorporating local invariant keypoint priors directly into deep semantic features can mitigate shape deformations and color shifts. By utilizing the Scale-Invariant Feature Transform (SIFT) \citep{sift}, we construct a local texture prior that is inherently invariant to scale, rotation, and minor lighting shifts. We project the semantic deep features (extracted via an EfficientNetB0 backbone \citep{efficientnet}) and the handcrafted SIFT descriptors into a unified latent manifold.

Rather than utilizing naive feature concatenation, we implement a novel \textbf{two-token attention} block. Given that our bimodal sequence length is compressed to only two (one global semantic token and one local texture token), the attention layer functions as a module for \textbf{compact multimodal reasoning}. This \textbf{lightweight attention fusion} mechanism realizes \textbf{token-space compression}, enabling relational gating at a fraction of standard Transformer complexity. This allows the local texture descriptors to modulate and reweight the semantic deep features under visual ambiguity.
...
We demonstrate that replacing the large token sequence of classical Transformers with a bimodal \textbf{two-token attention} mechanism is sufficient for \textbf{compact multimodal reasoning}. Specifically, projecting the global semantic features and local texture features into a compressed token-space (one semantic token, one texture token) allows the bimodal network to perform \textbf{token-space compression}. This \textbf{lightweight attention fusion} reduces the attention computational complexity from $O(N^2)$ to $O(4)$, enabling a compact MHA block with $<$263K parameters while still yielding measurable accuracy gains over naive concatenation (Section~\ref{sec:experiments}).
...
\paragraph{Bimodal Attention Fusion.} We shape the aligned embedding vectors into a bimodal \textbf{two-token sequence} of length 2:
\begin{equation}
...
\paragraph{Multimodal Token-Space Compression.} A core scientific insight of this architecture is that a highly restricted token-space---specifically, a two-token sequence representing the bimodal semantic and local texture modalities---is sufficient for \textbf{compact multimodal reasoning}. While conventional multimodal transformers typically leverage hundreds of spatial or patch-based tokens, EcoTexture AI demonstrates that performing \textbf{token-space compression} and utilizing a \textbf{lightweight attention fusion} mechanism achieves high representative capacity. This bimodal \textbf{two-token attention} allows query-conditioned context modulation at a fraction of the computational footprint.
...
The controlled benchmark focuses on parameter-efficient adaptation under low-data conditions. Frozen baselines are intentionally used to isolate the contribution of multimodal fusion under constrained trainable parameter budgets. By freezing the pre-trained feature extraction backbones, we ensure a fair and direct comparison of the architectural efficiency and representational capacity of the bimodal fusion layers themselves, without the confounding factor of full-backbone optimization dynamics.
...
Bimodal Attention ($d_k=256$) & EfficientNetB0 & SIFT (K=300) & MHA (4 heads, $d_k$=256) & 86.25\% \\
\textbf{EcoTexture AI (Ours)} & EfficientNetB0 & SIFT (K=300) & MHA (4 heads, $d_k$=64) & \textbf{87.06\%} \\
\bottomrule
\end{tabular}
\end{table}

The ablation results confirm that each component adds measurable value: (1) the SIFT branch via standard concatenation contributes $+1.35$ points over the pure fine-tuned semantic baseline; (2) replacing concatenation with the two-token cross-attention block provides a further $+1.56$ point gain (total $+2.91$ points). Additionally, we examine the effect of the key projection dimension $d_k$ in the two-token attention block. Scaling down $d_k$ from 256 to 64 not only reduces the parameter footprint but also yields a $+0.81\%$ improvement in test accuracy (from 86.25\% to 87.06\%), showing that lower-dimensional projections act as a regularizer in this highly compressed bimodal bottleneck. Critically, the attention block adds only 263K parameters ($<5.4\%$ of total model parameters), demonstrating that the Multimodal Token-Space Compression principle achieves non-trivial modality interaction at negligible parameter cost. Comparing the frozen and fine-tuned baselines also illustrates that the two-stage training protocol (Section~\ref{sec:training}) avoids catastrophic forgetting while enabling meaningful backbone adaptation.
...
Author contributions anonymized for peer review.
...
The implementation source code is available at \url{https://github.com/anonymous/EcoTexture-AI} (Anonymized for peer review).
